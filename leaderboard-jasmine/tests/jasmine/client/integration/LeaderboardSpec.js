...
describe("Point Assignment", function () {
  ...
  it("should give a player 5 points when he is selected and the button(give 5 points) is pressed", function () {
  ...
  it("should quit a player 5 points when he is selected and the button(quit 5 points) is pressed", function () {
    var graceInitialPoints = Players.findOne({name: "Grace Hopper"}).score;
    $("#decrem").click();
    expect(Players.findOne({name: "Grace Hopper"}).score).toBe(graceInitialPoints - 5);
  });
...
describe("No add player,when is not loggued", function () {
  it("No add Player when submit is pressed", function () {
